...
\begin{schedule}{STK}
	...
	DDD & $t^{DDD} = \begin{cases}
		\sdl{s}{\attr{DCL}}{\tmax} & \text{if}\quad \attr{DANX}\neq\undef \land \attr{DCL}\neq\undef \\
		\obs{ev}{\attr{CID},\attr{DDD}}{t_0} & \text{else} \end{cases}$ 
		& where\par
		$s=\begin{cases} \attr{DANX} & \text{if}\quad \attr{DLDD}=\undef \\
				\attr{DLDD}+\attr{DCL} & \text{else} \end{cases}$ \\
	\hline
	DED & $\vec{t}^{DED} = \begin{cases}
					\undef & \text{if}\quad \attr{DRP}=\undef \\
					\vec{t}^{DDD}+\attr{DRP} & \text{else} \end{cases}$ & \\
	\hline
	DPD & $\vec{t}^{DPD} = \begin{cases}
					\undef & \text{if}\quad \attr{DPP}=\undef \\
					\vec{t}^{DDD}+\attr{DPP} & \text{else} \end{cases}$ & \\
	\hline
	SDD & $t^{SDD} = \obs{ev}{\attr{CID},\attr{SDD}}{t_0}$ & \\
	\hline
	SED & $t^{SED} = t^{SDD}+\attr{DRP}$ \\
	\hline
	SSD & $\vec{t}^{SSD} = t^{SDD}+\attr{SSP}$ \\
	\hline
	RDD & $t^{RDD} = \obs{ev}{\attr{CID},\attr{RDD}}{t_0}$ & \\
	\hline
	RED & $t^{RED} = t^{RDD}+\attr{RRP}$ \\
	\hline
	RPD & $\vec{t}^{RPD} = t^{RDD}+\attr{RSP}$ \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
\begin{functions}{STK}
	...
	DDD & $0.0$ & {$\begin{aligned}
		\svar{DLDD}{t^+} &= t \\
		\svar{Dpa}{t^+} &= \obs{rf}{i}{t} \\
		\svar{Sd}{t^+} &= t \end{aligned}$}\par
		where\par
			$i = concat(\text{\attr{CID},"\_DPD"})$\\
	\hline
	...
	RDD & $0.0$ & {$\begin{aligned}
		\svar{Xa}{t^+} &= \obs{rf}{i}{t} \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \par
		where\par
			$i = concat(\text{\attr{CID},"\_RXA"})$\\
	\hline
	RED & $0.0$ & {$\begin{aligned}
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	RPD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\attr{RPA}\svar{Xa}{t}$ & 
			{$\begin{aligned}
				\svar{Qt}{t^+} &= \svar{Qt}{t}-\svar{Xa}{t} \\
				\svar{Xa}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
