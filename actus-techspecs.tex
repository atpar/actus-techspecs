...
The ACTUS dictionary defines attribute $\attr{CTS}$ which captures the child contract(s) as part of the parent contract's set of attributes. Thereby, attribute $\attr{CTS}$ is of type \verb'ContractReference[]' with \verb'ContractReference' a reference to a child contract (in JSON notation)
...
We will use the following notation to query reference objects from the $\attr{CTS}$ attribute

\[
	\attr{CTS}_{role}^{type}(i)
\]

where $role$ and $type$ filter the reference objects in $\attr{CTS}$ according to the respective values of fields \verb'type' and \verb'role'. For brevity, we will omit the index parameter $i$ which indicates that we address the first (and usually only) reference object queried.

\begin{example}[Underlying MarketObject-reference] The MarketObject reference of a simple Underlying e.g. to an Option contract is referenced as $\attr{CTS}_{Underlying}^{MarketObjectIdentifier}(1)$ or, in short form, as $\attr{CTS}_{Underlying}^{MarketObjectIdentifier}$.
\end{example}

\begin{example}[FirstLeg Contract-reference] The Contract object representing the first leg e.g. to a Swaps contract is referenced as $\attr{CTS}_{FirstLeg}^{Contract}(1)$ or, in short form, as $\attr{CTS}_{FirstLeg}^{Contract}$.
\end{example}
...
\begin{example}['ev'-Observer] The future events, w.r.t. time $t$, of the \textit{first leg} (i.e. child contract with \verb'Role'=$\texttt{FirstLeg}$) of a SWAPS contract with $\attr{CNTRL}=PFL$ (i.e. \textit{pay first leg}) can be evaluated as $\cldev{\attr{CTS}_{FirstLeg}^{Contract}}{t}{\attr{CNTRL}=RPL}$.
\end{example}

\begin{example}['sv'-Observer] The current state, w.r.t. time $t$, of state variable $\svar{Nt}{}$ of the \textit{first leg} (i.e. child contract with \verb'Role'=$\texttt{FirstLeg}$) of a SWAPS contract with \attr{CNTRL}=RFL (i.e. \textit{receive first leg}) can be evaluated as $\cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t}{Nt}{\attr{CNTRL}=RPA}$.
\end{example}

\begin{example}['ca'-Observer] The contract attribute \attr{MOC} of the child contract \textit{Child} (i.e. child contract with \verb'Role'=$\texttt{Underlying}$) of an OPTNS contract can be evaluated as $\cldca{\attr{CTS}_{Underlying}^{Contract}}{\attr{MOC}}$.
\end{example}
...
\begin{schedule}{SWAPS}
	...
	where for any two events $x_t^k\in\{e_t^{k,1}\}$, $y_s^l\in\{e_s^{l,2}\}$ we have $x_t^k=y_s^l \iff t=s \land k=l$, $\triangle$ is the \textit{distinct union}-operator, and $x_t^k+y_s^l=z_\tau^m$ with $\tau=t=s$, $m=k=l\in\{\text{IED,IP,PR}\}$ indicates that any two congruent events of type IED, IP, or PR are \textit{merged} into a new \textit{aggregate} event (see payoff and state transition function below).
		& with\par
			{$\begin{aligned}
				\{e_t^{k,1}\} &= \cldev{\attr{CTS}_{FirstLeg}^{Contract}}{t_0}{\attr{CNTRL}=r^{(1)}} \\
				\{e_s^{l,2}\} &= \cldev{\attr{CTS}_{SecondLeg}^{Contract}}{t_0}{\attr{CNTRL}=r^{(2)}} \\
...
% ---------------------- table: swaps states ----------------------
\begin{states}{SWAPS}
	$\svar{Md}{}$ & $\svar{Md}{t_0}=\max(\cldsvs{\attr{CTS}_{FirstLeg}^{Contract}}{t_0}{Tmd}, \cldsvs{\attr{CTS}_{SecondLeg}^{Contract}}{t_0}{Tmd})$ & \\
	\hline
  	$\svar{Ipac}{}$ & $\svar{Ipac}{t_0} = \cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t_0}{Ipac}{\attr{CNTRL}=r^{(1)}} + \cldsv{\attr{CTS}_{SecondLeg}^{Contract}}{t_0}{Ipac}{\attr{CNTRL}=r^{(2)}}$
			& with\par
			{$\begin{aligned}
				r^{(1)} &= \begin{cases} RPA & \text{if}\quad \attr{CNTRL}=RFL \\
					RPL & \text{else} \end{cases} \\
...
\begin{functions}{SWAPS}
	AD & \pof{AD}{PAM} & {$\begin{aligned}
				\svar{Ipac}{t^+} &= \cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(1)}} \\
						&+ \cldsv{\attr{CTS}_{SecondLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(2)}} \\
				\svar{Feac}{t^+} &= \begin{cases} \svar{Feac}{t^-} + \yfr{\svar{Sd}{t^-}}{t}\svar{Nt}{t^-}\attr{FER} & \text{if} \quad \attr{FEB}=\text{'N'} \\
					\frac{\yfr{t^-}{t}}{\yfr{t^-}{t^+}}\attr{FER} & \text{else} \end{cases} \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	PRD & $X_{\attr{CUR}}^{\attr{CURS}}(t) ( (-1)\attr{PPRD}$ \par
				 $\qquad + \cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(1)}}$ \par
				 $\qquad + \cldsv{\attr{CTS}_{SecondLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(2)}})$
		& {$\begin{aligned}
				\svar{Ipac}{t^+} &= \cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(1)}} \\
						&+ \cldsv{\attr{CTS}_{SecondLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(2)}} \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	...
	$z_\tau^m$ & $\fev{x_\tau^m}+\fev{y_\tau^m}$
		& {$\begin{aligned}
				\svar{Ipac}{t^+} &= \cldsv{\attr{CTS}_{FirstLeg}^{Contract}}{t}{Ipac}{\attr{CNTRL}=r^{(1)}} \\						&+ \cldsv{SecondLeg}{t}{Ipac}{\attr{CNTRL}=r^{(2)}} \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
...
\begin{schedule}{CAPFL}
	...
	for all events $x_t^k\in\cldev{\attr{CTS}_{Underlying}^{Contract}}{t_0}{\attr{CNTRL}=RPA}$, $y_s^l\in\cldev{\attr{CTS}_{Underlying}^{Contract}}{t_0}{\attr{CNTRL}=RPA,\attr{RRLC}=\attr{RRLC},\attr{RRLF}=\attr{RRLF}}$ with $t=s \land k=l=IP$. That is, any two congruent events of the child-contract schedule, evaluated once without $\attr{RRLC},\attr{RRLF}$ defined and once with the attributes defined, which are of type IP are \textit{merged} into a new \textit{aggregate} event (see payoff and state transition function below). & \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
% ---------------------- table: capfl states ----------------------
\begin{states}{CAPFL}
	$\svar{Md}{}$ & $\svar{Md}{t_0}=\max(\cldsv{\attr{CTS}_{Underlying}^{Contract}}{t_0}{Tmd}{\attr{CNTRL}=RPA},$ \par
			$\qquad \cldsv{\attr{CTS}_{Underlying}^{Contract}}{t_0}{Tmd}{\attr{CNTRL}=RPA,\attr{RRLC}=\attr{RRLC},\attr{RRLF}=\attr{RRLF}})$ & \\
	\hline
  	$\svar{Ipac}{}$ & $\svar{Ipac}{t_0} = \sgn abs($ \par
			$\qquad\cldsv{\attr{CTS}_{Underlying}^{Contract}}{t_0}{Ipac}{\attr{CNTRL}=r^{(1)}}$\par
			$\qquad - \cldsv{\attr{CTS}_{Underlying}^{Contract}}{t_0}{Ipac}{\attr{CNTRL}=RPA,\attr{RRLC}=\attr{RRLC},\attr{RRLF}=\attr{RRLF}})$
			& \\
	\hline
  	$\svar{Prf}{}$ & & Same as PAM \\
	\hline
...
\begin{functions}{CAPFL}
	AD & \pof{AD}{PAM} & {$\begin{aligned}
				\svar{Ipac}{t^+} &= \sgn  abs(\cldsv{\attr{CTS}_{Underlying}^{Contract}}{t}{Ipac}{\attr{CNTRL}=RPA} \\
						&- \cldsv{\attr{CTS}_{Underlying}^{Contract}}{t}{Ipac}{\attr{CNTRL}=RPA, \\
						&\qquad \attr{RRLC}=\attr{RRLC},\attr{RRLF}=\attr{RRLF}}) \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	PRD & $X_{\attr{CUR}}^{\attr{CURS}}(t) ( (-1)\attr{PPRD} + \sgn  abs($ \par
				 $\qquad \cldsv{\attr{CTS}_{Underlying}^{Contract}}{t}{Ipac}{\attr{CNTRL}=RPA} - $ \par
				 $\qquad \cldsv{\attr{CTS}_{Underlying}^{Contract}}{t}{Ipac}{\attr{CNTRL}=RPA,$ \par
...
\begin{schedule}{OPTNS}
	...
	XD & $t^{XD} = \begin{cases} \svar{Xd}{t_0} & \text{if}\quad \svar{Xd}{t_0} \neq \undef\\
		...
		and\par
		$i = \attr{CTS}_{CoveredContract}^{Contract}(i)$ \\
	\hline
...
\begin{functions}{OPTNS}
	...
	MD & 0.0 & {$\begin{aligned}
			...
			{$\begin{aligned}
				...
				S_t &= \obs{rf}{\cldca{\attr{CTS}_{Underlying}^{Contract}}{MOC}}{t} \end{aligned}$} \\
	\hline
	XD & 0.0 & {$\begin{aligned}
					...
					\svar{Sd}{t^+} &= t \end{aligned}$}\par
with\par
			$S_t=\obs{rf}{\cldca{\attr{CTS}_{Underlying}^{Contract}}{MOC}}{t}$ \\
	\hline
...
\begin{states}{CEG}
	$\svar{Md}{}$ & $\svar{Md}{t_0}=\begin{cases} \attr{MD} & \text{if}\quad \attr{MD}\neq\undef \\
				...
				& with\par
					{$\begin{aligned}
						...
						\{e_t^{k,i}\} &= \cldev{\attr{CTS}_{CoveredContract}^{Contract}(i)}{t_0}\\
						n &= \mid \attr{CTS}_{CoveredContract}^{Contract} \mid \end{aligned}$} \\
	\hline
	...
	where\par
	$I=\attr{CTS}_{CoveredContract}^{Contract}$ \\
	\hline
	$\svar{Feac}{}$ & $\svar{Feac}{t_0} = \begin{cases} 0.0 & \text{if} \quad \attr{FER}=\undef \\
					\attr{FEAC} & \text{else if} \quad \attr{FEAC} \neq \undef \\
...
\begin{states}{CEC}
	...
	$\svar{Nt}{}$ & $\svar{Nt}{t_0} = \begin{cases}
			...
			{$\begin{aligned}
				...
				I &= \attr{CTS}_{CoveredContract}^{Contract} \\
				J &= \attr{CTS}_{CoveringContract}^{Contract}\end{aligned}$} \\
	\hline
	$\svar{Sd}{}$ & & Same as PAM \\
\end{states}
...
\subsection{CERTF: Certificate}\label{sec:certf}


% ---------------------- table: certf schedule ----------------------
\begin{schedule}{CERTF}
	AD & & Same as PAM \\
	\hline
	ID & $t^{ID} = \attr{ID}$ & \\
	\hline
	IED & $t^{IED} = \begin{cases}
		\undef & \text{if}\quad \attr{IED}=\undef \\
		\attr{IED} & \text{else} \end{cases}$ &  \\
	\hline
	CFD & $t^{CFD} = \begin{cases}
		\undef & \text{if}\quad \attr{CPANX}=\undef \wedge \attr{CPCL}=\undef \\
		\sdl{s}{\attr{CPCL}}{T} & \text{else} \end{cases}$
		& where\par
		$s=\begin{cases} \attr{CPANX} & \text{if}\quad \attr{CPANX}\neq\undef \\
				\attr{ID}+\attr{CPCL} & \text{else} \end{cases}$\par
		and\par
		$T=\begin{cases} \attr{MD} & \text{if}\quad \attr{MD}\neq\undef \\
			\tmax & \text{else} \end{cases}$ \\
	\hline
	CPD & $t^{CPD} = \begin{cases} \undef & \text{if}\quad t^{CFD} = \undef\\
		t^{CFD} + \attr{STP} & \text{else} \end{cases}$ &  \\
	\hline
	RFD & $t^{RFD} = \begin{cases}
		\undef & \text{if}\quad \attr{RDANX}=\undef \wedge \attr{RDCL}=\undef \\
		\sdl{s}{\attr{RDCL}}{T} & \text{else} \end{cases}$
	   & where\par
		$s=\begin{cases} \attr{RDANX} & \text{if}\quad \attr{RDANX}\neq\undef \\
				\attr{ID}+\attr{RDCL} & \text{else} \end{cases}$
		and\par
		$T=\begin{cases} \attr{MD} & \text{if}\quad \attr{MD}\neq\undef \\
			\tmax & \text{else} \end{cases}$ \\
	\hline
	XO & $t^{XO}=\obs{ev}{\attr{CID},\attr{XO}}{t}$ & \\
	\hline
	XD & $t^{XD} = \begin{cases} \undef & \text{if}\quad t^{RFD} = \undef \\
				t^{RFD}\setminus\attr{MD} + \attr{XP} & \text{else if}\quad \attr{MD}\neq\undef \\
				t^{RFD} + \attr{XP} & \text{else} \end{cases}$ &  \\
	\hline
	RPD & $t^{RPD} = \begin{cases} \undef & \text{if}\quad t^{RFD} = \undef\\
		t^{RFD} + \attr{STP} & \text{else} \end{cases}$ &  \\
	\hline
	TD & $\obs{ev}{i,\attr{TD}}{t_0}$ &  \\
	\hline 
	MD & $t^{MD} = \begin{cases}
		\undef & \text{if}\quad \attr{MD}=\undef \\
		\attr{MD} & \text{else} \end{cases}$ &  \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
% ---------------------- table: certf states ----------------------
\begin{states}{CERTF}
	$\svar{Qt}{}$ & $\svar{Qt}{t_0} = \attr{QT}$ & \\
	\hline	
	$\svar{Xo}{}$ & $\svar{Xo}{t_0} = \begin{cases}
		\attr{XO} & \text{if}\quad \attr{XO}\neq\undef \\
		\attr{0} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Xq}{}$ & $\svar{Xq}{t_0} = \begin{cases}
		\attr{XQ} & \text{if}\quad \attr{XQ}\neq\undef \\
		\attr{0} & \text{else} \end{cases}$ & \\
	\hline
	$\svar{Xa}{}$ & $\svar{Xa}{t_0} = \begin{cases}
		\attr{XA} & \text{if}\quad \attr{XA}\neq\undef \\
		\attr{0} & \text{else} \end{cases}$ & \\
	\hline
	$\svar{Mrf}{}$ & $\svar{Mrf}{t_0} = \begin{cases}
		\attr{MRF} & \text{if}\quad \attr{MRF}\neq\undef \\
		\attr{1.0} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Adf}{}$ & $\svar{Adf}{t_0} = \begin{cases}
		\attr{ADF} & \text{if}\quad \attr{ADF}\neq\undef \\
		\attr{1.0} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Lcd}{}$ & $\svar{Lcd}{t_0} = \begin{cases}
		\attr{LCD} & \text{if}\quad \attr{LCD}\neq\undef \\
		\attr{ID} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Caf}{}$ & $\svar{Caf}{t_0} = \begin{cases}
		\attr{CAF} & \text{if}\quad \attr{CAF}\neq\undef \\
		\attr{0.0} & \text{else} \end{cases}$ & \\
	\hline
	$\svar{Prf}{}$ & & Same as PAM \\
	\hline
	$\svar{Sd}{}$ & & Same as PAM \\
...
% ---------------------- table: certf functions ----------------------
\begin{functions}{CERTF}
	AD & \pof{AD}{PAM} & $\svar{Sd}{t^+} = t$ \\
	\hline
	ID & 0.0 & {$\begin{aligned}
		\svar{Qt}{t^+} &= \attr{QT}  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IED & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\attr{IPR}$ & $\svar{Sd}{t^+} = t$ \\
	\hline
	CFD & 0.0 & {$\begin{aligned}
					\svar{Caf}{t^+} &= f^{CP}()  \\
					\svar{Lcd}{t^+} &= t  \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \par 
					with Coupon Amount formula\par
					$f^{CP} = \begin{cases}
						\yfr{\svar{Lcd}{t}}{t}\attr{NPR}\times\attr{CPRT} & \text{if}\quad \attr{CPTP}='FIX' \\
						\obs{rf}{\attr{CID}}{t}\yfr{\svar{Lcd}{t}}{t}\attr{NPR}\times\attr{CPRT} & \text{if}\quad \attr{CPTP}='FCN' \\
						\obs{rf}{\attr{concat(CID,'-',CPFX)}}{t} & \text{else} \end{cases}$
					 \\
	\hline
	CPD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\svar{Caf}{t}$ &  {$\begin{aligned}
		\svar{Caf}{t^+} &= 0.0 \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	RFD & 0.0 & {$\begin{aligned}
					\svar{Xa}{t^+} &= f^{RA}()  \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \par 
					with $f^{RA}$ the contract Redemption Amount formula, 
					e.g.\par
					$f^{RA}=\frac{\obs{rf}{\attr{CTS}_{Underlying}^{MOC}}{t-\attr{FIP}}}{\obs{rf}{\attr{CTS}_{Underlying}^{MOC}}{\attr{ID}}}\attr{NPR}\svar{Mrf}{t}\svar{Adf}{t}$ \par
					
					 \\
	\hline
	XO & 0.0 & {$\begin{aligned}
		\svar{Xo}{t^+} &= min(\svar{Qt}{t},\svar{Xo}{t}+ev^{QT})  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \par
		with $ev^{QT}$ the exercised quantity as per XO event. \\
	\hline
	XD & 0.0 & {$\begin{aligned}
		\svar{Xq}{t^+} &= \svar{Xo}{t}  \\
		\svar{Xo}{t^+} &= 0  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	RPD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Xq}{t}\svar{Xa}{t}$ & {$\begin{aligned}
		\svar{Qt}{t^+} &= \svar{Qt}{t}-\svar{Xq}{t} \\
		\svar{Xq}{t^+} &= 0.0 \\		
		\svar{Xa}{t^+} &= 0.0 \\
		\svar{Prf}{t^+} &=\begin{cases} \svar{Prf}{t} & \text{if } \svar{Md}{t} = \undef \wedge \svar{Td}{t} = \undef \\
			\text{matured} & \text{else if } \svar{Md}{t} \neq \undef \\
			\text{terminated} & \text{else} \end{cases}	\\		
		\svar{Sd}{t^+} &= t \end{aligned}$}	\\
	\hline
	TD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\svar{Xa}{t}$ & {$\begin{aligned}
		\svar{Qt}{t^+} &= 0  \\
		\svar{Td}{t^+} &= t \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
