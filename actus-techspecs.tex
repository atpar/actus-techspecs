...
\begin{schedule}{CERTF}
	...
	EXE & $t^{EXE} = \begin{cases} \undef & \text{if}\quad t^{REF} = \undef \\
				t^{REF}\setminus\attr{MD} + \attr{EXP} & \text{else if}\quad \attr{MD}\neq\undef \\
				t^{REF} + \attr{EXP} & \text{else} \end{cases}$ &  \\
	\hline
...
\begin{functions}{CERTF}
	...
	COF & 0.0 & {$\begin{aligned}
					...
					$f^{COF} = \begin{cases}
						\yfr{\svar{Lcd}{t}}{t}\attr{NPR}\times\attr{CORT} & \text{if}\quad \attr{COTP}='FIX' \\
						\obs{rf}{\attr{CID}}{t}\yfr{\svar{Lcd}{t}}{t}\attr{NPR}\times\attr{CORT} & \text{if}\quad \attr{COTP}='FCN' \\
						\obs{rf}{\attr{concat(CID,'-',COFX)}}{t} & \text{else} \end{cases}$
					 \\
	\hline
	COP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\svar{Caf}{t}$ &  {$\begin{aligned}
		\svar{Caf}{t^+} &= 0.0 \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	...
	EXE & 0.0 & {$\begin{aligned}
		...
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	REP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Xq}{t}\svar{Xa}{t}$ & {$\begin{aligned}
