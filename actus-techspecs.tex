...
\begin{schedule}{STK}
	...
	\hline
	REX & $t^{REX} = t^{REF}+\attr{RERP}$ \\
	\hline
	REP & $\vec{t}^{REP} = t^{REF}+\attr{REPP}$ \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
\begin{schedule}{CERTF}
	...
	\hline
	REX & $t^{REX} = \begin{cases} \undef & \text{if}\quad t^{REF} = \undef \\
				t^{REF}\setminus\attr{MD} + \attr{RERP} & \text{else if}\quad \attr{MD}\neq\undef \\
				t^{REF} + \attr{RERP} & \text{else} \end{cases}$ &  \\
	\hline
