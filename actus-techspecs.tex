...
\begin{states}{LAM}
	...
	\hline
	$\svar{Ipcb}{}$ & $\svar{Ipcb}{t_0}= \begin{cases} 0.0 & \text{if}\quad t_0<\attr{ISS} \\
...
\begin{schedule}{STK}
	...
	TD & & Same as PAM \\
	\hline
	DIF & $t^{DIF} = \begin{cases}
		\sdl{s}{\attr{DICL}}{\tmax} & \text{if}\quad \attr{DIANX}\neq\undef \land \attr{DICL}\neq\undef \\
		\obs{ev}{\attr{CID},\attr{DIF}}{t_0} & \text{else} \end{cases}$ 
		& where\par
		$s=\begin{cases} \attr{DIANX} & \text{if}\quad \attr{LDIFD}=\undef \\
				\attr{LDIFD}+\attr{DICL} & \text{else} \end{cases}$ \\
	\hline
	DIX & $\vec{t}^{DIX} = \begin{cases}
					\undef & \text{if}\quad \attr{DIRP}=\undef \\
					\vec{t}^{DIX}+\attr{DIRP} & \text{else} \end{cases}$ & \\
	\hline
	DIP & $\vec{t}^{DIP} = \begin{cases}
					\undef & \text{if}\quad \attr{DIPP}=\undef \\
					\vec{t}^{DIF}+\attr{DIPP} & \text{else} \end{cases}$ & \\
	\hline
	SPF & $t^{SPF} = \obs{ev}{\attr{CID},\attr{SPF}}{t_0}$ & \\
	\hline
	SPS & $\vec{t}^{SPS} = t^{SPF}+\attr{SPSP}$ \\
	\hline
	REF & $t^{REF} = \obs{ev}{\attr{CID},\attr{REF}}{t_0}$ & \\
	\hline
	REX & $t^{REX} = t^{REF}+\attr{REXP}$ \\
	\hline
	REP & $\vec{t}^{REP} = t^{REF}+\attr{RESP}$ \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
\begin{functions}{STK}
	...
	TD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn \attr{PTD}$
		& $\svar{Sd}{t^+}=t$ \\
	\hline
	DIF & $0.0$ & {$\begin{aligned}
		\svar{LDIFD}{t^+} &= t \\
		\svar{Dipa}{t^+} &= \obs{rf}{i}{t} \\
		\svar{Sd}{t^+} &= t \end{aligned}$}\par
		where\par
			$i = concat(\text{\attr{CID},"\_DIP"})$\\
	\hline
	DIX & $0.0$ & {$\begin{aligned}
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	DIP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Dipa}{t}$ & 
			{$\begin{aligned}
				\svar{Dipa}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	SPF & $0.0$ & {$\begin{aligned}
		\svar{Spra}{t^+} &= \obs{rf}{i}{t} \\
		\svar{Lspfd}{t^+} &= t \end{aligned}$}\par
		where\par
			$i = concat(\text{\attr{CID},"\_SRA"})$\\
	\hline
	SPS & $0.0$ & {$\begin{aligned}
		\svar{Qt}{t^+} &= \svar{Spra}{t^-}\times\svar{Qt}{t^-} \\
		\svar{Spra}{t^+} &= 0.0 \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	REF & $0.0$ & {$\begin{aligned}
		\svar{Exa}{t^+} &= \obs{rf}{i}{t} \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \par
		where\par
			$i = concat(\text{\attr{CID},"\_REXA"})$\\
	\hline
	REX & $0.0$ & {$\begin{aligned}
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	REP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\attr{REPA}\svar{Exa}{t}$ & 
			{$\begin{aligned}
				\svar{Qt}{t^+} &= \svar{Qt}{t}-\svar{Exa}{t} \\
				\svar{Exa}{t^+} &= 0.0 \\
				\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
...
\begin{schedule}{CERTF}
	...
	IED & $t^{IED} = \begin{cases}
		...
		\attr{IED} & \text{else} \end{cases}$ &  \\
	\hline
	COF & $t^{COF} = \begin{cases}
		\undef & \text{if}\quad \attr{COANX}=\undef \wedge \attr{COCL}=\undef \\
		\sdl{s}{\attr{COCL}}{T} & \text{else} \end{cases}$
		& where\par
		$s=\begin{cases} \attr{COANX} & \text{if}\quad \attr{COANX}\neq\undef \\
				\attr{ISD}+\attr{COCL} & \text{else} \end{cases}$\par
		and\par
		$T=\begin{cases} \attr{MD} & \text{if}\quad \attr{MD}\neq\undef \\
			\tmax & \text{else} \end{cases}$ \\
	\hline
	COP & $t^{COP} = \begin{cases} \undef & \text{if}\quad t^{COF} = \undef\\
		t^{COF} + \attr{COPP} & \text{else} \end{cases}$ &  \\
	\hline
	REF & $t^{REF} = \begin{cases}
		\undef & \text{if}\quad \attr{REANX}=\undef \wedge \attr{RECL}=\undef \\
		\sdl{s}{\attr{RECL}}{T} & \text{else} \end{cases}$
	   & where\par
		$s=\begin{cases} \attr{REANX} & \text{if}\quad \attr{REANX}\neq\undef \\
				\attr{ISD}+\attr{RECL} & \text{else} \end{cases}$
		and\par
		$T=\begin{cases} \attr{MD} & \text{if}\quad \attr{MD}\neq\undef \\
			\tmax & \text{else} \end{cases}$ \\
	\hline
	EXO & $t^{EXO}=\obs{ev}{\attr{CID},\attr{EXO}}{t}$ & \\
	\hline
	EXE & $t^{EXE} = \begin{cases} \undef & \text{if}\quad t^{REF} = \undef \\
				t^{REF}\setminus\attr{MD} + \attr{REXP} & \text{else if}\quad \attr{MD}\neq\undef \\
				t^{REF} + \attr{REXP} & \text{else} \end{cases}$ &  \\
	\hline
	REP & $t^{REP} = \begin{cases} \undef & \text{if}\quad t^{REF} = \undef\\
		t^{REF} + \attr{REPP} & \text{else} \end{cases}$ &  \\
	\hline
...
\begin{states}{CERTF}
	...
	$\svar{Adf}{}$ & $\svar{Adf}{t_0} = \begin{cases}
		...
		\attr{1.0} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Lcfd}{}$ & $\svar{Lcfd}{t_0} = \begin{cases}
		\attr{LCD} & \text{if}\quad \attr{LCD}\neq\undef \\
		\attr{ID} & \text{else} \end{cases}$ & \\
	\hline	
	$\svar{Coaf}{}$ & $\svar{Coaf}{t_0} = \begin{cases}
		\attr{COAF} & \text{if}\quad \attr{COAF}\neq\undef \\
...
\begin{functions}{CERTF}
	AD & \pof{AD}{PAM} & $\svar{Sd}{t^+} = t$ \\
	\hline
	ISS & 0.0 & {$\begin{aligned}
		\svar{Qt}{t^+} &= \attr{QT}  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IED & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\attr{IPR}$ & $\svar{Sd}{t^+} = t$ \\
	\hline
	COF & 0.0 & {$\begin{aligned}
					\svar{Coaf}{t^+} &= f^{COF}()  \\
					\svar{Lcofd}{t^+} &= t  \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \par 
					with Coupon Amount formula\par
					$f^{COF} = \begin{cases}
						\yfr{\svar{Lcofd}{t}}{t}\attr{NPR}\times\attr{CORT} & \text{if}\quad \attr{COTP}='FIX' \\
						\obs{rf}{\attr{CID}}{t}\yfr{\svar{Lcofd}{t}}{t}\attr{NPR}\times\attr{CORT} & \text{if}\quad \attr{COTP}='FCN' \\
						\obs{rf}{\attr{concat(CID,'-',COFX)}}{t} & \text{else} \end{cases}$
					 \\
	\hline
	COP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\svar{Coaf}{t}$ &  {$\begin{aligned}
		\svar{Coaf}{t^+} &= 0.0 \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	REF & 0.0 & {$\begin{aligned}
					\svar{Exa}{t^+} &= f^{REF}()  \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \par 
					with $f^{REF}$ the contract Redemption Amount formula, 
					e.g.\par
					$f^{REF}=\frac{\obs{rf}{\attr{CTS}_{Underlying}^{MOC}}{t-\attr{FIP}}}{\obs{rf}{\attr{CTS}_{Underlying}^{MOC}}{\attr{ID}}}\attr{NPR}\cdot\attr{RAT}\cdot\svar{Mrf}{t}\svar{Adf}{t}$ \par
					
					 \\
	\hline
	EXO & 0.0 & {$\begin{aligned}
		\svar{Exo}{t^+} &= min(\svar{Qt}{t},\svar{Exo}{t}+ev^{QT})  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \par
		with $ev^{QT}$ the exercised quantity as per EXO event. \\
	\hline
	EXE & 0.0 & {$\begin{aligned}
		\svar{Exq}{t^+} &= \svar{Exo}{t}  \\
		\svar{Exo}{t^+} &= 0  \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	REP & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Exq}{t}\svar{Exa}{t}$ & {$\begin{aligned}
		\svar{Qt}{t^+} &= \svar{Qt}{t}-\svar{Exq}{t} \\
		\svar{Exq}{t^+} &= 0.0 \\		
		\svar{Exa}{t^+} &= 0.0 \\
		...
		\svar{Sd}{t^+} &= t \end{aligned}$}	\\
	\hline
	TD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Qt}{t}\svar{Exa}{t}$ & {$\begin{aligned}
		\svar{Qt}{t^+} &= 0  \\
		\svar{Td}{t^+} &= t \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	MD & 0.0 & {$\begin{aligned}
		\svar{Exq}{t^+} &= \svar{Qt}{t}  \\
		\svar{Md}{t^+} &= t \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	CE & \pof{CE}{PAM} & \stf{AD}{OPTNS} \\
