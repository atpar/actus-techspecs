...
\begin{schedule}{CERTF}
	...
	\hline
	RD & $t^{RD} = \begin{cases}
		\undef & \text{if}\quad \attr{RDANX}=\undef \wedge \attr{RDCL}=\undef \wedge \attr{MD}=\undef \\
		\sdl{s}{\attr{RDCL}}{\attr{MD}} & \text{else} \end{cases}$
	   & where\par
	...
	\hline
	TD & $t^{TD}=\obs{ev}{\attr{CID},\attr{TD}}{t}$ & \\
	\hline
	XD & $t^{XD} = \begin{cases} \undef & \text{if}\quad t^{RD} = \undef\\
				t^{RD} + \attr{XP} & \text{else} \end{cases}$ &  \\
	\hline
	STD & $t^{STD} = \begin{cases} \undef & \text{if}\quad t^{RD} = \undef\\
		t^{RD} + \attr{STP} & \text{else} \end{cases}$ &  \\
	\hline
	CE & & Same as PAM \\
\end{schedule}
...
% ---------------------- table: certf states ----------------------
\begin{states}{CERTF}
	$\svar{Xo}{}$ & $\svar{Xo}{t_0} = \attr{XO}$ & \\
	\hline	
	$\svar{Xq}{}$ & $\svar{Xq}{t_0} = \attr{XQ}$ & \\
	\hline
	$\svar{Xa}{}$ & $\svar{Xa}{t_0} = \attr{XA}$ & \\
	\hline
	$\svar{Td}{}$ & $\svar{Td}{t_0} = \attr{TD}$ & \\
	\hline
	$\svar{Prf}{}$ & & Same as PAM \\
	\hline
	$\svar{Sd}{}$ & & Same as PAM \\
...
\begin{functions}{CERTF}
	...
	ID & 0.0 & {$\begin{aligned}
		...
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	IED & $\sgn\svar{Qt}{t}\attr{IP}$ & $\svar{Sd}{t^+} = t$ \\
	\hline
	MD & 0.0 & {$\begin{aligned}
		\svar{Xo}{t^+} &= \svar{Qt}{t}  \\
		\svar{Md}{t^+} &= t \\
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	RD & 0.0 & {$\begin{aligned}
					\svar{Xa}{t^+} &= f^{RA}(\mathcal{A},\mathcal{S}_t,t)  \\
					\svar{Sd}{t^+} &= t \end{aligned}$} \par 
					with $f^{RA}$ the contract Redemption Amount formula, 
					$\mathcal{A}$ the attributes and $\mathcal{S}_t$ state as per current time $t$.  \par
					E.g.\par
					$f^{RA}=\frac{\obs{rf}{\attr{CTST}_{Underlying}^{MOC}}{t}}{\obs{rf}{\attr{CTST}_{Underlying}^{MOC}}{\attr{RDANX}}}\attr{NP}\svar{Mrf}{t}\svar{Adf}{t}$ \par
	...
	XD & 0.0 & {$\begin{aligned}
		...
		\svar{Sd}{t^+} &= t \end{aligned}$} \\
	\hline
	STD & $X_{\attr{CUR}}^{\attr{CURS}}(t)\sgn\svar{Xq}{t}\svar{Xa}{t}$ & {$\begin{aligned}
